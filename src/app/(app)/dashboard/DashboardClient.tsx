...
const NumberTicker = dynamic(() => import("@/components/motion/NumberTicker"), { ssr: false });
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <GlowCard key={stat.label} delay={i * 0.1}>
            <div className="group relative glass-card-solid rounded-xl p-5 hover:border-brand-primary/20 transition-all duration-300 cursor-default">
              ...
              <p className="text-2xl font-display font-bold text-text-main mb-1">
                {stat.label === "Conversion Rate" ? (
                  stat.display
                ) : (
                  <NumberTicker value={stat.value} prefix={stat.label === "Total Revenue" ? "₹" : ""} />
                )}
              </p>
              <div className="flex items-center gap-1.5">
                <span className={`text-xs font-medium ${stat.change.startsWith("+") || stat.change === "—" ? "text-success" : "text-error"}`}>
                  {stat.change}
                </span>
            ...
            <ChitiCard padding="md" glass hover>
              ...
              <div className="space-y-1">
                {attentionItems.slice(0, 6).map((item, i) => (
                  <Link key={i} href={item.href} className="flex items-center gap-3 py-2.5 px-3 -mx-3 rounded-lg hover:bg-surface-2/40 transition-colors group">
